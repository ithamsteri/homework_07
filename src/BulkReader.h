...
public:
  ...
  void subscribe(const std::function<void(const Bulk&)>& function) { subscribers_.push_back(function); }
...
private:
  std::vector<std::function<void(const Bulk&)>> subscribers_;
  std::istream& istream_;
  size_t bulk_size_;

  /// Send bulk to all subscribers
  /// @param bulk bulk for subscribers
  void notify(Bulk& bulk) const
  {
